"""Base class to represent different types of documents"""

from __future__ import annotations
import logging
from abc import ABC, abstractmethod
from typing import Optional, Collection, Collection, Mapping, Any
from crossref.restful import Works
from papis import api as Papis
from papis.commands.add import run as PapisAdd
from scraper.base_classes.taglist import TagList
...
class Document:
    ...
    def add_to_library(self, confirm=True, link=False, **kwargs):
        PapisAdd(
            self.files,
            self.info | {"tags": str(self.tags)} | kwargs,
            confirm=confirm,
            link=link,
        )
        return self
...
class DocumentType(ABC):
    ...
    @abstractmethod
    def download(
        cls: DocumentType, document: Document, outdir=Constants.DOWNLOAD_DIR, **kwargs
    ) -> Collection[str]:
        ...

    @classmethod
    def run(cls: DocumentType, doc: Document) -> None:
        """
    ...
    @classmethod
    def scrape(cls: DocumentType, document: Document) -> dict:
        """
    ...
    def _consolidate_doi_data(cls, papis_data, cref_data):
        ...
        return {
            key: val
            for key, val in cref_data.items()
            if key not in papis_data and key.lower() not in papis_data
        } | papis_data
